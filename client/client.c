...
uint64_t download_test(char *src_ip, char *dst_ip);
void upload_test(uint32_t id);
void consultar_resultados(char *ip, int udp_port, uint32_t id_measurement);
void exportar_json(uint64_t bw_down, struct BW_result resultado, double rtt_idle, double rtt_down, double rtt_up, const char *src_ip, const char *dst_ip);
...
void consultar_resultados(char *ip, int udp_port, uint32_t id_measurement) {
    ...
    // Desempaquetar
    struct BW_result r;
    if (unpackResultPayload(&r, buffer, n) < 0) {
    ...
    for (int i = 0; i < NUM_CONN; i++) {
        if (r.conn_duration[i] > 0.0) {
            double throughput = (double)r.conn_bytes[i] / r.conn_duration[i];
            printf("Conexión %d: %.2f bytes/s (duración %.3f s)\n", i, throughput, r.conn_duration[i]);
        }
    }
